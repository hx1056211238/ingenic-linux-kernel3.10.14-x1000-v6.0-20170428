...
#include "rndis.c"
#include "u_ether.h"
#ifdef CONFIG_USB_ANDROID_WEBCAM
#include "webcam.c"
#endif
...
static int hid_init(struct android_usb_function *f,
			struct usb_composite_dev *cdev)
{
...
static void hid_cleanup(struct android_usb_function *f)
{
	ghid_cleanup();
	return;
}

static int hid_bind_config(struct android_usb_function *f,
		struct usb_configuration *c)
{
			    ...
			    const char *buff, size_t size)
{
	...
	if (enabled && !dev->enabled) {
		...
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->enable)
				f->enable(f);	//may be have wait userspace access disable(adb)
		}
		android_enable(dev);
		dev->enabled = true;
		...
		list_for_each_entry(f, &dev->enabled_functions, enabled_list) {
			if (f->disable)
				f->disable(f);	//may be have balance enable(adb)
		}
		dev->enabled = false;
	} else {
...
#ifdef CONFIG_USB_ANDROID_HID
	err = platform_driver_probe(&hidg_plat_driver,
