...
static void sfc_mode(struct sfc *sfc, int channel, int value)
{
	unsigned int tmp;
...
static void __attribute__((unused)) sfc_set_cmd_length(struct sfc *sfc, unsigned int value)
{
	if(value == 1){
		unsigned int tmp;
		tmp = sfc_readl(sfc,SFC_DEV_CONF);
...
static void sfc_transfer_direction(struct sfc *sfc, int value)
{
...
static unsigned int __attribute__((unused)) sfc_fifo_num(struct sfc *sfc)
{
	unsigned int tmp;
...
static unsigned int __attribute__((unused)) sfc_get_fsm(struct sfc *sfc)
{
	return sfc_readl(sfc,SFC_FSM);
}
static void sfc_set_addr_length(struct sfc *sfc, int channel, unsigned int value)
{
	unsigned int tmp;
	tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
	tmp &= ~(ADDR_WIDTH_MSK);
...
static void sfc_cmd_enble(struct sfc *sfc, int channel, unsigned int value)
{
	if(value == ENABLE) {
		unsigned int tmp;
		tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
...
static void sfc_data_en(struct sfc *sfc, int channel, unsigned int value)
{
	if(value == 1) {
		unsigned int tmp;
		tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
...
static void sfc_phase_format(struct sfc *sfc, int channel, unsigned int value)
{
	if(value == 1) {
		unsigned int tmp;
		tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
...
static void sfc_write_cmd(struct sfc *sfc, int channel, unsigned int value)
{
	unsigned int tmp;
	tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
...
static void sfc_dev_data_dummy_bytes(struct sfc *sfc, int channel, unsigned int value)
{
	unsigned int tmp;
	tmp = sfc_readl(sfc, SFC_TRAN_CONF(channel));
