...
int tcu_as_timer_config(struct tcu_device *tcu)
{
	...
	if (tcu->init_level)
		...
		tcu_set_pwm_output_init_level(tcu->id, 0);
 ...
 * set default output level, but TCU2 doesn't. Only for M200 and X1000*/
static int tcu_set_fun_pin(struct tcu_device *tcu)
{
	int ret = -EIO;
	switch (tcu->id) {
	case 0:
		ret = jzgpio_set_func(GPIO_PORT_C, GPIO_FUNC_0, BIT(25));
		break;
	case 1:
		ret = jzgpio_set_func(GPIO_PORT_C, GPIO_FUNC_1, BIT(26));
		break;
	case 2:
		ret = jzgpio_set_func(GPIO_PORT_C, GPIO_FUNC_1, BIT(27));
		break;
	case 3:
		ret = jzgpio_set_func(GPIO_PORT_B, GPIO_FUNC_2, BIT(6));
		break;
	case 4:
		ret = jzgpio_set_func(GPIO_PORT_C, GPIO_FUNC_0, BIT(24));
		break;
	default:
...
static int tcu_set_pin_to_default_level(struct tcu_device *tcu)
{
	...
	switch (tcu->id) {
	case 0:
		ret = jzgpio_set_func(GPIO_PORT_C, (tcu->init_level ?
				GPIO_OUTPUT1 : GPIO_OUTPUT0), BIT(25));
		break;
	case 1:
		ret = jzgpio_set_func(GPIO_PORT_C, (tcu->init_level ?
				GPIO_OUTPUT1 : GPIO_OUTPUT0), BIT(26));
		break;
	case 2:
		ret = jzgpio_set_func(GPIO_PORT_C, (tcu->init_level ?
				GPIO_OUTPUT1 : GPIO_OUTPUT0), BIT(27));
		break;
	case 3:
		ret = jzgpio_set_func(GPIO_PORT_B, (tcu->init_level ?
				GPIO_OUTPUT1 : GPIO_OUTPUT0), BIT(6));
		break;
	case 4:
		ret = jzgpio_set_func(GPIO_PORT_C, (tcu->init_level ?
				GPIO_OUTPUT1 : GPIO_OUTPUT0), BIT(24));
		break;
	default:
		pr_warn("TCU:%d, has not default pin,"
				" Please check your code\n",
